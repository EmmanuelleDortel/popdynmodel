      ...
      for (s in 1:ntax) {
        for (j in 1:nreg[s]) {
          for (n in 1:nperiod) {
            ...
            N_lambda[s,reg[s,j],n] <- n_reg[s,reg[s,j],n] * prod(cal.N_lambda[s,reg[s,j],start[n]:end[n]])
          ...
          for (t in 2:ntime) {
            ...
            N_intlambda[s,reg[s,j],t] <- n_regt[s,reg[s,j],t] * sum(cal.N_id[s,reg[s,j],1:nidreg[s,j],t]) / max(1,sum(cal.N[s,reg[s,j],1:nidreg[s,j],t]))
            cal.N_lambda[s,reg[s,j],t] <- N_intlambda[s,reg[s,j],t] + (1 - n_regt[s,reg[s,j],t])
            for (i in 1:nidreg[s,j]) {
              n_zreg[s,reg[s,j],i,t] <- n_idt[s,idreg[j,i,s],t]
              cal.N_id[s,reg[s,j],i,t] <- N_intlambda_id[s,idreg[j,i,s],t] * N[s,idreg[j,i,s],t-1]
        ...
        for (i in 1:nid[s]) {
          for (n in 1:nperiod) {
            ...
            N_lambda_id[s,idtax[s,i],n] <-  n_id[s,idtax[s,i],n] * prod(cal.Nlambda_id[s,idtax[s,i],start[n]:end[n]])
        ...
        for(g in 1:ngui) {
          for (j in 1:nregui[g]) {
            for (n in 1:nperiod) {
              ...
              N_lambda_gui[g,regui[g,j],n] <- n_gui[g,regui[g,j],n] * prod(cal.N_lambda_gui[g,regui[g,j],start[n]:end[n]])
            ...
            for (t in 2:ntime) {
              ...
              N_intlambda_gui[g,regui[g,j],t] <- n_guit[g,regui[g,j],t] * prod(cal.Nlambda_tax[g,regui[g,j],1:ngtax[g,j],t])^(1 / max(1,sum(n_tax[g,regui[g,j],1:ngtax[g,j],t])))
              cal.N_lambda_gui[g,regui[g,j],t] <- N_intlambda_gui[g,regui[g,j],t] + (1 - n_guit[g,regui[g,j],t])
      ...
      for (s in 1:ntax) {
        for (j in 1:nreg[s]) {
          for (n in 1:nperiod) {
            ...
            B_lambda[s,reg[s,j],n] <- b_reg[s,reg[s,j],n] * prod(cal.B_lambda[s,reg[s,j],start[n]:end[n]])
          ...
          for (t in 2:ntime) {
            ...
            B_intlambda[s,reg[s,j],t] <- b_regt[s,reg[s,j],t] * sum(cal.B_id[s,reg[s,j],1:nidreg[s,j],t]) / max(1,sum(cal.B[s,reg[s,j],1:nidreg[s,j],t]))
            cal.B_lambda[s,reg[s,j],t] <- B_intlambda[s,reg[s,j],t] + (1 - b_regt[s,reg[s,j],t])
            for (i in 1:nidreg[s,j]) {
              b_zreg[s,reg[s,j],i,t] <- b_idt[s,idreg[j,i,s],t]
              cal.B_id[s,reg[s,j],i,t] <- B_intlambda_id[s,idreg[j,i,s],t] * B[s,idreg[j,i,s],t-1]
        ...
        for (i in 1:nid[s]) {
          for (n in 1:nperiod) {
            ...
            B_lambda_id[s,idtax[s,i],n] <-  b_id[s,idtax[s,i],n] * prod(cal.Blambda_id[s,idtax[s,i],start[n]:end[n]])
        ...
        for(g in 1:ngui) {
          for (j in 1:nregui[g]) {
            for (n in 1:nperiod) {
              ...
              B_lambda_gui[g,regui[g,j],n] <- b_gui[g,regui[g,j],n] * prod(cal.B_lambda_gui[g,regui[g,j],start[n]:end[n]])
            ...
            for (t in 2:ntime) {
              ...
              B_intlambda_gui[g,regui[g,j],t] <- b_guit[g,regui[g,j],t] * prod(cal.Blambda_tax[g,regui[g,j],1:ngtax[g,j],t])^(1 / max(1,sum(b_tax[g,regui[g,j],1:ngtax[g,j],t])))
              cal.B_lambda_gui[g,regui[g,j],t] <- B_intlambda_gui[g,regui[g,j],t] + (1 - b_guit[g,regui[g,j],t])
