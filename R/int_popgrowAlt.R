      ...
      for (s in 1:ntax) {
        for (j in 1:nreg[s]) {
          ...
          for (t in 2:ntime) {
            ...
            N_intlambda[s,reg[s,j],t] <- n_regt[s,reg[s,j],t] * sum(cal.lambda[s,reg[s,j],1:nidreg[s,j],t]) / (n_regt[s,reg[s,j],t] * sum(cal.N[s,reg[s,j],1:nidreg[s,j],t]) + (1- n_regt[s,reg[s,j],t]))
            cal.N_intlambda[s,reg[s,j],t] <- N_intlambda[s,reg[s,j],t] + (1 - n_regt[s,reg[s,j],t])
            for (i in 1:nidreg[s,j]) {
              cal.lambda[s,reg[s,j],i,t] <- N_intlambda_id[s,idreg[j,i,s],t] * N[s,idreg[j,i,s],t-1]
              cal.N[s,reg[s,j],i,t] <- n_idt[s,idreg[j,i,s],t] * N[s,idreg[j,i,s],t-1]
      ...
      for (s in 1:ntax) {
        for (j in 1:nreg[s]) {
          ...
          for (t in 2:ntime) {
            ...
            B_intlambda[s,reg[s,j],t] <- b_regt[s,reg[s,j],t] * sum(cal.Blambda[s,reg[s,j],1:nidreg[s,j],t]) / (b_regt[s,reg[s,j],t] * sum(cal.B[s,reg[s,j],1:nidreg[s,j],t]) + (1- b_regt[s,reg[s,j],t]))
            cal.B_intlambda[s,reg[s,j],t] <- B_intlambda[s,reg[s,j],t] + (1 - b_regt[s,reg[s,j],t])
            for (i in 1:nidreg[s,j]) {
              cal.Blambda[s,reg[s,j],i,t] <- B_intlambda_id[s,idreg[j,i,s],t] * B[s,idreg[j,i,s],t-1]
              cal.B[s,reg[s,j],i,t] <- b_idt[s,idreg[j,i,s],t] * B[s,idreg[j,i,s],t-1]
