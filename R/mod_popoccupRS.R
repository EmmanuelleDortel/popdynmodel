...
#' @examples
#' \dontrun{
#' mcmc.out <- mod_popoccupRS(riverfish,
#' var_id = "pop_id",
#' var_tmp = "year",
#' var_cnt = "headcount",
#' var_pas = "pass",
...
#' var_guild = "family",
#' var_pro = "fishing")
#' mcmc.out <- mod_popoccupRS(riverfish[riverfish$taxa == "eel",],
#' var_id = "pop_id",
#' var_tmp = "year",
#' var_cnt = "headcount",
#' var_pas = "pass")
#' }
mod_popoccupRS <- function(df, var_id, var_tmp, var_cnt, var_pas, var_pro=NULL, var_tax=NULL, var_reg=NULL, var_guild=NULL, period=NULL, timestep=1, save_parameters=NULL, n_chain=3, n_iter=102000, n_thin=1000, n_burnin=2000) {
  #-----------------------------------------------------------------------------
  ...
  df <- do.call(int_checkfunction, list(df,
                                        vars_in_df=syms(c(var_id, var_tmp, var_tax, var_cnt, var_reg, var_guild, var_pas, var_pro)),
                                        vars_na=syms(c(var_id, var_tmp, var_tax, var_pas, var_pro)),
                                        vars_numeric=syms(c(var_tmp, var_cnt, var_pas)),
  ...
  datamodel <- do.call(int_datamodel, list(df, occup=TRUE, grow=FALSE, modenv=FALSE, modenvG=FALSE, alt=FALSE, RS=TRUE, timestep, period, var_id, var_tmp, var_tax, var_pres, var_reg, var_guild, var_cnt=NULL, var_wei=NULL, var_surf=NULL, var_pro, var_pas, var_envO=NULL, var_envP=NULL, var_envC=NULL, var_grow=NULL, var_det))
  code <- do.call(int_popoccup, list(occup=TRUE,modenv=FALSE,RS=TRUE,var_envO=NULL,var_envP=NULL,var_envC=NULL,var_guild,var_det))
